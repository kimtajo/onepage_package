...
class onepage_team extends WidgetHandler
{
	...
	function _getDocumentItems($args)
	{
		...
		$output = executeQueryArray('widgets.onepage_team.getCategories',$obj);
	...
	function _getImageItems($args)
	{
		...
		foreach($tmp_document_list as $oDocument)
		{
			...
			$content_item = new onepageTeamItem($browser_title);
...
class onepageTeamItem extends Object
{
	...
	function setFirstRealImage($content, $widget_path, $skin){
		...
		$this->add('realImage', (!empty($matches[0][1]))?$matches[0][1]:sprintf('%sskins/%s/images/img_1.jpg', $widget_path, $skin));
	}
