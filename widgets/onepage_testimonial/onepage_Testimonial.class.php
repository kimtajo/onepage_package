...
class onepage_testimonial extends WidgetHandler
{
	...
	function proc($args)
	{
		...
		if(!$args->module_srls)
		{
			...
			$output = executeQueryArray('widgets.onepage_testimonial.getMids', $obj);
			if($output->data)
			{
	...
	function _getDocumentItems($args)
	{
		...
		$output = executeQueryArray('widgets.onepage_testimonial.getNewestDocuments', $obj);
		...
		if(count($output->data))
		{
			...
			for($i=0,$c=count($document_srls);$i<$c;$i++)
			{
				...
				$content_item = new onepageTtestimonialItem( $args->module_srls_info[$module_srl]->browser_title );
...
class onepageTtestimonialItem extends Object
{
	var $browser_title = null;
	var $contents_link = null;
	var $domain = null;

	function onepageTtestimonialItem($browser_title='')
	{
